	...
	'video': {
            ...
            'playlist': function(pflag) {
                ...
                try{
                     ...
                     for (var i = 0, len = list.length; i < len; i++) {
                             ...
                             var title=(xyplay.part==i)?array[0]+"":array[0];
                            var style=(xyplay.part==i)?xyplay.play.style.play_on:'';
                            l =`<li class="playlist" style="${style}" onclick="YZM.video.part(${i});">${title}</li></d>`;
                            
                             p =`<option class="playlist-item"  value="${i}" ${(xyplay.part==i)?"selected":""}>${title}</option>`;
        
	                         config.video.push(array[1]);
                                 config.vname.push(array[0]);
                                $(".playlist-show").append(l);
                                $("#playlist").append(p);
                     }
     ...
     //检测是否能播放           
      $.ajax({
            ...
            beforeSend: function () {loadIndex = layer.load(0, { shade: [0.1,'#fff'],time:10*1000}); },
